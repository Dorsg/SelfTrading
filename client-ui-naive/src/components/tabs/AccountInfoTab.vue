<template>
  ...
  const formatCurrency = (value) => {
    return ` $ ${value.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`; // Adding $ and commas
  };
  ...
  onMounted(async () => {
    ...
    if (data) {
      ...
      statistics.value = [
        ...
        { label: 'Excess Liquidity', value: formatCurrency(data.excess_liquidity) },
        { label: 'Gross Position Value', value: formatCurrency(data.gross_position_value) }
      ];
    }
